...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m5155constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m5155constructorimpl = Result.m5155constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m5155constructorimpl = Result.m5155constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m5158exceptionOrNullimpl(m5155constructorimpl) != null) {
            m5155constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
        return (String) m5155constructorimpl;
    }
