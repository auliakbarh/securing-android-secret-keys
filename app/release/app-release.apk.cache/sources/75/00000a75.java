...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo955rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer composer, int i) {
        ...
        if (this.color.getValue().m2638unboximpl() != Color.Companion.m2664getUnspecified0d7_KjU()) {
            mo957defaultColorWaAFU9c = this.color.getValue().m2638unboximpl();
        } else {
            mo957defaultColorWaAFU9c = rippleTheme.mo957defaultColorWaAFU9c(composer, 0);
        }
        composer.endReplaceableGroup();
        RippleIndicationInstance mo955rememberUpdatedRippleInstance942rkJo = mo955rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, SnapshotStateKt.rememberUpdatedState(Color.m2618boximpl(mo957defaultColorWaAFU9c), composer, 0), SnapshotStateKt.rememberUpdatedState(rippleTheme.rippleAlpha(composer, 0), composer, 0), composer, (i & 14) | ((i << 12) & 458752));
        EffectsKt.LaunchedEffect(mo955rememberUpdatedRippleInstance942rkJo, interactionSource, new Ripple$rememberUpdatedInstance$1(interactionSource, mo955rememberUpdatedRippleInstance942rkJo, null), composer, ((i << 3) & 112) | 520);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        composer.endReplaceableGroup();
        return mo955rememberUpdatedRippleInstance942rkJo;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof Ripple) {
            ...
            return this.bounded == ripple.bounded && Dp.m4877equalsimpl0(this.radius, ripple.radius) && Intrinsics.areEqual(this.color, ripple.color);
        }
        return false;
    }

    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m4878hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}