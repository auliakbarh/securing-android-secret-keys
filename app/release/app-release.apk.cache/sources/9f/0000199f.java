...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public final long m4657getSizeNHjbRc() {
        return this.size;
    }

    /* renamed from: setSize-uvyYCjk  reason: not valid java name */
    public final void m4658setSizeuvyYCjk(long j) {
        this.size = j;
    }
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float f) {
        ...
        this.alpha = f;
        this.size = Size.Companion.m2466getUnspecifiedNHjbRc();
    }

    @Override // android.text.style.CharacterStyle
    public void updateDrawState(TextPaint textPaint) {
        Shader mo2597createShaderuvyYCjk;
        Intrinsics.checkNotNullParameter(textPaint, "textPaint");
        AndroidTextPaint_androidKt.setAlpha(textPaint, this.alpha);
        if (this.size == Size.Companion.m2466getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m2454equalsimpl0(pair.getFirst().m2463unboximpl(), this.size)) {
            mo2597createShaderuvyYCjk = this.shaderBrush.mo2597createShaderuvyYCjk(this.size);
        } else {
            mo2597createShaderuvyYCjk = pair.getSecond();
        }
        textPaint.setShader(mo2597createShaderuvyYCjk);
        this.cachedShader = TuplesKt.to(Size.m2446boximpl(this.size), mo2597createShaderuvyYCjk);
    }
}