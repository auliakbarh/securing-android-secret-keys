...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long j) {
        ...
        this.selectionStart = TextRange.m4401getMinimpl(j);
        this.selectionEnd = TextRange.m4400getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m4401getMinimpl = TextRange.m4401getMinimpl(j);
        int m4400getMaximpl = TextRange.m4400getMaximpl(j);
        if (m4401getMinimpl < 0 || m4401getMinimpl > text.length()) {
            throw new IndexOutOfBoundsException("start (" + m4401getMinimpl + ") offset is outside of text region " + text.length());
        }
        if (m4400getMaximpl < 0 || m4400getMaximpl > text.length()) {
            throw new IndexOutOfBoundsException("end (" + m4400getMaximpl + ") offset is outside of text region " + text.length());
        }
        if (m4401getMinimpl > m4400getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + m4401getMinimpl + " > " + m4400getMaximpl);
        }
    }
    ...
    public final TextRange m4544getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m4391boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release  reason: not valid java name */
    public final long m4545getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int i, int i2) {
        ...
        long m4546updateRangeAfterDeletepWDy79M = EditingBufferKt.m4546updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m4401getMinimpl(m4546updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4400getMaximpl(m4546updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m4546updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4546updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m4397getCollapsedimpl(m4546updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
                return;
            }
            this.compositionStart = TextRange.m4401getMinimpl(m4546updateRangeAfterDeletepWDy79M2);
            this.compositionEnd = TextRange.m4400getMaximpl(m4546updateRangeAfterDeletepWDy79M2);
        }
    }
