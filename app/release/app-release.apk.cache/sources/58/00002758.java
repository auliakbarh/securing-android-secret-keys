...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m6715constructorimpl = SegmentOrClosed.m6715constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m6715constructorimpl = SegmentOrClosed.m6715constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m6720isClosedimpl(m6715constructorimpl)) {
                Segment m6718getSegmentimpl = SegmentOrClosed.m6718getSegmentimpl(m6715constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m6718getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m6718getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m6718getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m6718getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m6718getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m6718getSegmentimpl(m6715constructorimpl);
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m6718getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m6718getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m6718getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
