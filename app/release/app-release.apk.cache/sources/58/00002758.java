...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m6712constructorimpl = SegmentOrClosed.m6712constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m6712constructorimpl = SegmentOrClosed.m6712constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m6717isClosedimpl(m6712constructorimpl)) {
                Segment m6715getSegmentimpl = SegmentOrClosed.m6715getSegmentimpl(m6712constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m6715getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m6715getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m6715getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m6715getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m6715getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m6715getSegmentimpl(m6712constructorimpl);
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m6715getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m6715getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m6715getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
