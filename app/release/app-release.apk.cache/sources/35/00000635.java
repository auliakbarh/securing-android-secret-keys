...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int i) {
        ...
        if (lazyGridItemInfo2 != null) {
            if (this.state.isVertical$foundation_release()) {
                m4990getXimpl = IntOffset.m4991getYimpl(lazyGridItemInfo2.mo611getOffsetnOccac());
            } else {
                m4990getXimpl = IntOffset.m4990getXimpl(lazyGridItemInfo2.mo611getOffsetnOccac());
            }
            return Integer.valueOf(m4990getXimpl);
        }
        return null;
    }
    ...
    private final int calculateLineAverageMainAxisSize(LazyGridLayoutInfo lazyGridLayoutInfo, final boolean z) {
        int m5032getWidthimpl;
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (z) {
                        m5032getWidthimpl = IntSize.m5031getHeightimpl(visibleItemsInfo.get(i).mo612getSizeYbymL2g());
                    } else {
                        m5032getWidthimpl = IntSize.m5032getWidthimpl(visibleItemsInfo.get(i).mo612getSizeYbymL2g());
                    }
                    i4 = Math.max(i4, m5032getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
